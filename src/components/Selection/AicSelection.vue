    ...
    >
      ...
      <div class="charts-list-container">
        <div v-if="isLoading" class="loading-state">
          <n-spin size="small" />
          <span class="loading-text">正在加载AIC...</span>
        </div>
            ...
            <div class="section-charts" v-show="expandedSections.has(String(section))">
              ...
              >
                <div class="chart-info">
                  <div class="chart-name">{{ getChartMainName(chart.Local_Subject || chart.Subject || 'AIC') }}</div>
...
const charts = ref<OfficialAIC[]>([])
const chartsBySection = ref<{ [section: string]: OfficialAIC[] }>({})
...
const categoryTitle = computed(() => {
  return categoryTitles[props.activeCategory] || 'AIC'
})
...
const loadCharts = async () => {
  ...
  try {
    // 列表模式：按Section分组
    const sectionData = await getAicChartsBySection()
    ...
    console.error('Failed to load aic charts:', err)
    error.value = err instanceof Error ? err.message : '加载AIC失败'
    charts.value = []
