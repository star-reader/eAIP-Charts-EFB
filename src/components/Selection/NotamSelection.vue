    ...
    >
      ...
      <div class="charts-list-container">
        <div v-if="isLoading" class="loading-state">
          <n-spin size="small" />
          <span class="loading-text">正在加载NOTAM...</span>
        </div>
          ...
          >
            ...
            <div class="section-charts" v-show="expandedSections.has(String(section))">
              <div 
                ...
                :key="String(chart.Document)"
                class="chart-item section-chart-item"
                @click="handleChartSelect(chart)"
              >
                <div class="chart-info">
                  <div class="chart-name">{{ getChartMainName(chart.SeriesName) }}</div>
                </div>
              </div>
            </div>
          </div>
...
const charts = ref<OfficialNOTAM[]>([])
const chartsBySection = ref<{ [section: string]: OfficialNOTAM[] }>({})
...
const categoryTitle = computed(() => {
  return categoryTitles[props.activeCategory] || 'NOTAM'
})
...
const loadCharts = async () => {
  ...
  try {
    // 列表模式：按Section分组
    const sectionData = await getNotamChartsBySection()
    ...
    console.error('Failed to load notam charts:', err)
    error.value = err instanceof Error ? err.message : '加载NOTAM失败'
    charts.value = []
